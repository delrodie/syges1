...
class GestionImpression
{
     ...
     * Les décimales sont arrondies a  deux chiffres avant traitement.
     *
    ...
    function conversion($n,$cpt){
        ...
        $C=substr($n,0,1); $D=substr($n,1,1); $U=substr($n,2,1);
        //Initialisation
        $valeur='';
        ...
        //Conversion
        if($D==0){$valeur=$chiffre[$U];} 							// 1 �  10 (ne retourne pas "zéro" pour 0)
    ...
    public function nombre_en_lettre($montant)
    {
        ...
        if(!preg_match('/^[,\.0-9]+$/',$montant)){return false;}
            ...
            $retour='';
            // Paramétrage
            $devise=array("S"=>'Franc CFA', "P"=>'Francs CFA');						// devise "entière"
            ...
            $valeur["d"]=round("0.".$temp[1],2)*100;						// valeur décimale (arrondie �  2 chiffres)
            unset($temp);													// nettoyage
            // Conversion
            if($valeur["e"]==0 || $valeur["e"]==''){
                if($montant==0){ return "zero ".$devise["S"];}				//pas de décimales => retour : "zéro ..."
            ...
            elseif($valeur["e"]==1){$retour="un ".$devise["S"];}
                ...
                $retour=trim($retour).' '.$devise["P"];						// d'office pluriel, car 0 et 1 traités �  part
            }
            /* chiffres décimaux */
